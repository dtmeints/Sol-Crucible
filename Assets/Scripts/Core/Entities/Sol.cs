...
public class Sol : MonoBehaviour
{
    ...
    [SerializeField] Transform revealingMask;
    ...
    void OnEnable() {
        if (revealingMask) {
            revealingMask.transform.parent = null;
            revealingMask.position = transform.position;
        }
    }
    ...
    void Update() {
        ...
        if (Vector2.Distance(mainCamera.ScreenToWorldPoint(mousePos), transform.position) <= transform.localScale.x * 0.5f) {
